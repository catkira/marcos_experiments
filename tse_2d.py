"""Constructor for 2D TSE Imaging sequence."""
# %%
from math import pi
...
system = pp.Opts(
    adc_raster_time=1e-06,
    block_duration_raster=1e-06,
    grad_raster_time=1e-06,
    max_grad=100,
    grad_unit='mT/m',
    max_slew=150,
    slew_unit='T/m/s',
...
def constructor(
    ...
    grad_ro = pp.make_trapezoid(
        ...
        flat_time=np.round((adc_duration + gradient_correction) / 1e-6) * 1e-6,
    )
    ...
    grad_ro_pre = pp.make_trapezoid(
        ...
        system=system,
        area=grad_ro.area / 2,
        duration=np.round(pp.calc_duration(grad_ro) / 2 / 1e-6) * 1e-6,
    )
    ...
    pe_duration = np.round(pp.calc_duration(grad_ro) / 2 / 1e-6) * 1e-6
    ...
    ro_counter = 0
    pe_traj_idx = np.zeros((int(n_enc.y), ))
    pe_traj_mom = np.zeros((int(n_enc.y), ))
    ...
    n_total_trains = len(pe_1_trains)
...
encoding_dim = Dimensions(x=120, y=120, z=1)
etl = 6
seq, traj, _ = constructor(
    echo_time=20e-3,
    repetition_time=370e-3,
    etl=etl,
    gradient_correction=0e-6,
    rf_duration=200e-6,
    ro_bandwidth=35.6e3,
...
seq.plot(time_range=(0, 1.132))
seq.calculate_kspace()
seq.write('tse_2d_new.seq')
